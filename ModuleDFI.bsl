...
// Заполнение структуры КолонокОбъекта для последующей передачи в методы СздДинамическийСписок, СздТаблицаФормы.
// Используется если необходимо вывести все колонки на форму.
// Пример:  ПолучитьСтруктуруКолонокОбъекта("Справочники","Контрагенты");
//
// Параметры:
//	ТипОбъекта - Строка - Строковое представление типа объекта (РегистрыСведений, Справочники, Документы и.т.п)
//  ИмяОбъекта - Строка - Имя объекта (Контрагенты, Организации)
//
Функция ПолучитьСтруктуруКолонокОбъекта(ТипОбъекта,ИмяОбъекта) Экспорт
	ОбъектМетаданных= Метаданные.РегистрыСведений[ИмяОбъекта];
	СтруктураКолонок=Новый Структура;

	СтруктураСвойствОбъекта = Новый Структура("Измерения,СтандартныеРеквизиты,Реквизиты,Графы,Ресурсы,ПризнакиУчета,ПризнакиУчетаСубконто");
	ЗаполнитьЗначенияСвойств(СтруктураСвойствОбъекта,ОбъектМетаданных);

	Для Каждого ТекСвойство из СтруктураСвойствОбъекта цикл     
		Если ТекСвойство.Значение=Неопределено тогда
			Продолжить;
		КонецЕсли;

		Для каждого ТекПоле из ТекСвойство.Значение цикл
			СтруктураКолонок.Вставить(ТекПоле.Имя,ТекПоле.Синоним);
		КонецЦикла;
	КонецЦикла;
